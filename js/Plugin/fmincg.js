// https://raw.githubusercontent.com/gulfaraz/ml-util/master/fmincg.js

var mathjs = require("mathjs");
import { matrix } from 'mathjs';
import {vectorMultiplication} from '../util'
...
export function fmincg(f, X, options, P1, P2, P3, P4, P5, P6) {
    ...
    X = mathjs.matrix([...currentX._data]);

    i = i + ((length<0) ? 1 : 0);

    var s = mathjs.multiply(-1, df1);
    let d1_one = mathjs.dotMultiply(-1, mathjs.transpose(s));

    var d1 = mathjs.sum(mathjs.dotMultiply(
        d1_one,
        s
    ));
 ...
 */
        ...
        X = mathjs.matrix([...currentX]);
        i = i + ((length<0) ? 1 : 0);
        console.log('before d2');
                ...
                (
                    mathjs.larger(
                        ...
                        mathjs.add(
                            f1,
                            mathjs.dotMultiply(
                                mathjs.dotMultiply(z1, CONSTANT.RHO),
                                d1
                            )
                        )
                    )
                    || mathjs.larger(
                        d2,
                        mathjs.dotMultiply(
                            mathjs.dotMultiply(-1, CONSTANT.SIG),
                            d1
                        )
                    )
                )
...
/*                 var z2ProductSLength = 0;
                ...
                currentX = [...mathjs.matrix(X)._data];
                [ f2, df2 ] = updatedF(X);
